...
9. **Ajuste fino do Notebook 7 (EDOs de 2ª ordem com SymPy):**
   - Notebook era cópia quase literal do NB08 antigo, sem nenhuma saída executada — reescrito e reexecutado do zero (`jupyter nbconvert --execute`), sem erros.
   - Removida a nota de reorganização; numeração refeita: `## 7` + `### 7.1`/`7.2`/`7.3` + `#### 7.2.1`/`7.2.2`/`7.2.3`; exemplos `Exemplo 7.1`–`7.10` (antes "8.1"–"8.9", com uma duplicata: "Exemplo 8.2" aparecia duas vezes para exemplos diferentes); `Atividade 7.1`–`7.5` adicionadas distribuídas pelo corpo (padrão dos NB1–6).
   - Bugs de conteúdo corrigidos: dependência oculta de variável entre células nos plots de família de soluções (raízes complexas e raízes imaginárias puras — variáveis `c1`/`c2` usadas antes de serem definidas na própria célula, "funcionando" só por lixo deixado por uma célula anterior); typo no enunciado do Exemplo 7.5 ($y''+9y'=0$ → $y''+9y=0$, o código já usava a equação certa); **sinal trocado** na construção da equação passada ao `sp.dsolve` nos Exemplos 7.9 e 7.10 (o código resolvia $y''-3y'-4y=+8e^t\cos(2t)$ em vez de $-8e^t\cos(2t)$ pedido no enunciado — resultado da solução particular saía com o sinal invertido).
   - Exemplos 7.9 e 7.10 (antes 8.8/8.9) estavam rasos — célula de código vazia, resposta direta via `dsolve` sem o passo a passo (montar $Y(t)$, derivar, substituir, resolver o sistema) usado nos Exemplos 7.7/7.8 — desenvolvidos no mesmo padrão; o Exemplo 7.10 agora usa o princípio da superposição explicitamente, somando as soluções particulares dos Exemplos 7.7–7.9.
   - Aproveitado o resultado "estranho" do `sp.dsolve` no Exemplo 7.6 (raízes repetidas, coeficiente `0.25` como float) para uma nota explicando o efeito de ponto flutuante e mostrando a correção com `sp.Rational` — antes o notebook só dizia "Ops!" sem explicar a causa.
   - Adicionado um terceiro exercício sobre coeficientes indeterminados (o notebook só tinha exercícios de equações homogêneas, apesar de metade do conteúdo ser sobre não homogêneas) e a seção de **Referências** (Boyce e Diprima — mesma fonte dos exercícios, confirmada por comparação com o NB4).
   - Link da imagem do pêndulo padronizado para `raw.githubusercontent.com/.../refs/heads/main/...`; removidas 3 imagens não usadas da pasta `imagens/` (`diffs.png` e 2 imagens "ChatGPT Image...").
   - Notebook reexecutado do zero e commitado (commit `48ebd50`).
   - **Nota sobre a ferramenta de edição:** editar célula a célula com `NotebookEdit` em lote (várias chamadas independentes na mesma mensagem) corrompeu o notebook — "substituições" viraram inserções duplicadas ao lado da célula original, com tipos markdown/code trocados. Recuperado com `git checkout` e reconstrução do notebook inteiro via script Python manipulando o JSON diretamente (`nb['cells']`), depois reexecutado com `nbconvert`. Preferir esse caminho (script Python direto no JSON) para edições grandes/múltiplas num mesmo notebook.
   - **Revisão de conteúdo em 2026-09-22:** removida do notebook principal a célula "Questionário Moodle" (Questão 1, afirmativas I/II/III) da Seção 7.1 — conteúdo redundante, já coberto separadamente pelo `N07_Q1` dos arquivos de questões Moodle (item 10). Mais três bugs de conteúdo encontrados e corrigidos nessa revisão, distintos do bug de sinal do Exemplo 7.9 já corrigido no item acima:
     - Exemplo 7.1 (pêndulo): o texto chamava $\theta(t)=\cos(\sqrt{g/l}\,t)$ de "solução geral", mas essa é só uma instância particular (equivalente a $\theta_0=1$) — a solução geral de fato só aparece depois, na Atividade 7.1, com dois parâmetros ($\theta_0$, $\omega_0$). Corrigido para "uma solução".
     - Exemplo 7.9: o texto de confirmação após o `sp.dsolve` (célula logo após o resultado) transcrevia a solução particular sem o fator $e^t$ ($\frac{2}{13}sen(2t)+\frac{10}{13}cos(2t)$ em vez de $\frac{2}{13}e^t\,sen(2t)+\frac{10}{13}e^t\cos(2t)$) — só a célula de texto estava errada, o resultado do `sp.dsolve` já estava correto.
     - Exemplo 7.10 (superposição): **sinal trocado** no termo $sen(t)$ da equação passada ao `sp.dsolve` (`+2*sp.sin(t)` em vez de `-2*sp.sin(t)` na construção `eq = lado_esquerdo - lado_direito`) — o código resolvia $y''-3y'-4y=3e^{2t}-2\,sen(t)-8e^t\cos(2t)$ em vez do enunciado $3e^{2t}+2\,sen(t)-8e^t\cos(2t)$, então o resultado executado não batia com a fórmula de superposição escrita no texto logo acima (que estava certa). Corrigido e reexecutado; resultado agora bate com o texto.
     - Notebook reexecutado do zero (`jupyter nbconvert --execute`), sem erros; nenhuma outra saída (incluindo os gráficos) mudou.

10. **Questões Moodle do Notebook 7** (arquivos *gitignored* — `07_questoes_moodle*.xml` / `07_questoes_moodle_*.ipynb`, nunca entram em commit):
    - Conjunto criado em 2026-09-21: 5 questões de múltipla escolha cobrindo verificação de soluções de 2ª ordem (afirmativas I/II/III), raízes reais distintas, raízes complexas com parte real não nula (questão gráfica), raízes complexas puras, e coeficientes indeterminados com forçante polinomial+exponencial (`N07_Q1`…`N07_Q5`), nos 4 arquivos-padrão (`_moodle.xml`, `_alternativas.xml`, `_enunciados.ipynb`, `_resolvidas.ipynb`).
    - Q1 corresponde à Questão 1 já presente no corpo do notebook principal (célula de "Questionário Moodle" da Seção 7.1); Q2 e Q3 correspondem às Atividades 7.2 e 7.3.
    - **Q4** foi trocada, a pedido do professor, de uma questão conceitual sobre o efeito de ponto flutuante em `sp.dsolve` com raiz dupla (tema original da Atividade 7.4, sem gráfico natural) para uma **questão gráfica**: PVI $y''-2y'+5y=0$, $y(0)=1$, $y'(0)=3$ (raízes complexas $r=1\pm2i$, oscilação de amplitude crescente $y(t)=e^t(\cos2t+\operatorname{sen}2t)$), no mesmo esquema de imagem embutida em base64 das Q3/Q4 gráficas do NB05/NB06. Distratores: (b) parte real com sinal trocado (decai em vez de crescer); (c) frequência dobrada ($\mu=4$ em vez de $2$); (d) $c_2=1,5$ em vez de $1$ (erro ao aplicar $y'(0)$). O tema original de ponto flutuante saiu do questionário (Exemplo 7.6 continua coberto só no corpo do notebook principal).
    - **Q5** foi ajustada, a pedido do professor, para gerar um **sistema 3×3** na resolução (em vez do caso de 1 incógnita da Atividade 7.5, que usa forçante exponencial $2e^{3t}$). Passou por duas versões: primeiro com forçante polinomial quadrática ($Y(t)=At^2+Bt+C$); depois, a pedido, trocada para o palpite específico $Y(x)=Ax+B+Ce^{3x}$ (combinação de polinômio de grau 1 + exponencial, via superposição), mantendo a mesma equação homogênea $y''-3y'-4y=\dots$ (raízes $r=4,-1$, já familiares do Exemplo 7.7) com forçante $-8x+6-4e^{3x}$. Isso gera o sistema $-4A=-8$, $-3A-4B=6$, $-4C=-4$, cuja solução $A=2,B=-3,C=1$ foi verificada com `sp.dsolve` e por `sp.solve` no sistema. Os 3 distratores simulam erros plausíveis de montagem do sistema (esquecer o termo $-3Y'$ por completo, ou usar seu sinal trocado só na equação de $B$) e inversão de sinal de toda a forçante.
    - **Bug encontrado e corrigido na Questão 1** (também no notebook principal `07_EDOs_2a_ordem_com_sympy.ipynb`): a afirmativa (II) trazia `cos(6x)` em vez de `cos(3x)` — com o erro, nenhuma das 4 alternativas correspondia à combinação real de afirmativas verdadeiras/falsas (a questão ficava sem resposta válida). Corrigido para `cos(3x)` (verificado simbolicamente com SymPy: resíduo da EDO dá exatamente 0), tornando as três afirmativas verdadeiras e a alternativa correta **(d) Todas estão corretas**.
    - Todos os valores numéricos das alternativas foram calculados em Python/SymPy e conferidos contra a saída executada do `_resolvidas.ipynb` (`jupyter nbconvert --execute`, sem erros); alternativas mantidas no padrão enxuto (só a afirmação, sem justificar o valor errado — a explicação de cada distrator fica só no `_resolvidas.ipynb`).
    - `_moodle.xml` e `_alternativas.xml` idênticos no bloco de respostas, diferindo apenas no enunciado (completo × só o prompt "qual é verdadeira?"), como no padrão dos NB5/NB6. Frases de fechamento padronizadas para "Considerando o enunciado da Questão N..." (evitando referências tipo "acima" que só fazem sentido no arquivo completo).
    - Sem questão gráfica desta vez (todas as 5 são numéricas/conceituais); `<math>` MathML usado só para as equações diferenciais e fórmulas de solução geral, seguindo o padrão do NB04 (entidades HTML simples como `&sup2;`/`<sup>` para expoentes soltos no meio do texto).

## Próximos passos

- Seguir o ajuste fino notebook por notebook, a partir do **Notebook 8** (EDOs de 2ª ordem: métodos numéricos — atenção: é um dos dois notebooks que são só esqueletos, junto com o NB2).
...
- Notebooks 3, 4, 5, 6 e 7 já com ajuste fino feito e commitado (NB5 = commits `392b1bb`, `39e27bb`, `aae3210`; NB6 = commit `9efbaad`; NB7 = commit `48ebd50`). As questões Moodle do NB5 (item 6), NB6 (item 8) e NB7 (item 10) foram criadas/revisadas mas são arquivos *gitignored* — não entram em commit. O fix do typo `cos(6x)`→`cos(3x)` no notebook principal do NB7 (item 10) ainda está pendente de commit no working tree.
