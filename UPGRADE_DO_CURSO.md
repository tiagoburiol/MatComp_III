...
10. **Questões Moodle do Notebook 7** (arquivos *gitignored* — `07_questoes_moodle*.xml` / `07_questoes_moodle_*.ipynb`, nunca entram em commit):
    - Conjunto criado em 2026-09-21: 5 questões de múltipla escolha cobrindo verificação de soluções de 2ª ordem (afirmativas I/II/III), raízes reais distintas, raízes complexas puras, o efeito de ponto flutuante em `sp.dsolve` com raiz dupla (Exemplo 7.6), e coeficientes indeterminados com forçante polinomial (`N07_Q1`…`N07_Q5`), nos 4 arquivos-padrão (`_moodle.xml`, `_alternativas.xml`, `_enunciados.ipynb`, `_resolvidas.ipynb`).
    - Q1 corresponde à Questão 1 já presente no corpo do notebook principal (célula de "Questionário Moodle" da Seção 7.1); Q2–Q4 correspondem às Atividades 7.2–7.4.
